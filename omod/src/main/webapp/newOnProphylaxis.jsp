...
<i style="color: red"><spring:message code="drugorderexport.newOnProphylaxisMsg" /></i>

<br/><br/>

<b class="boxHeader" style="width: 100%"><spring:message
	code="drugorderexport.prophyNew" /></b>

<div>
<div
	style="background: #E2E4FF; border: 1px #808080 solid; padding: 0.5em; margin-bottom: 0em">
<div>
<form action="newOnProphylaxis.form" method="post" name=""
...
<c:if test="${fn:length(patients1)>0}">
    ...
    <c:if test="${gender=='Female'}">(<spring:message code="drugorderexport.female" />) </c:if>
     ...
     <spring:message code="drugorderexport.newProphylaxis" /> 
	...
	<table>
		<tr>
			<td>
			<form action="newOnProphylaxis.form?export=excel" method="post"><input type="hidden"
				name="linkId" value="${linkId }" /> <input type="hidden"
			...
			<td>
			<form action="newOnProphylaxis.form?export=pdf" method="post"><input
				type="hidden" name="linkId" value="${linkId }" /> <input
	...
	<table cellpadding="0" cellspacing="0" border="0" class="display"
		...
		<thead>
			<tr style="color: #666; font-weight: bold;">
				...
				<th><spring:message code="drugorderexport.starProphylaxis" /></th>
		...
		<tbody>
			<c:forEach var="p" items="${objectsList}" varStatus="i">
				<tr>
					<td>&nbsp; ${p[0].patientIdentifier}</td>
					<openmrs:hasPrivilege privilege="View Patient Names">
					<td>&nbsp; ${p[0].givenName }</td>
					<td>&nbsp; ${p[0].familyName}</td>
					</openmrs:hasPrivilege>
					<td>&nbsp; ${p[0].age}</td>
					<td>&nbsp;<img
						src="${pageContext.request.contextPath}/images/${p[0].gender == 'M' ? 'male' : 'female'}.gif" />
					</td>
					<td>&nbsp;${p[1]}</td>
					<td>&nbsp;${p[2]}</td>
					<td>&nbsp;${p[3]}</td>
					...
					<td>&nbsp; <a
						href="${pageContext.request.contextPath}/patientDashboard.form?patientId=${p[0].patientId}">View
					Dashboard</a></td>
					<td>&nbsp; 
					<openmrs:hasPrivilege privilege="View Patient Regimens History">
					<a
						href="${pageContext.request.contextPath}/module/drugorderexport/showPatientRegimens.form?patient=${p[0].patientId}"><spring:message
